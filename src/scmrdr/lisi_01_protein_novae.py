...
"""
lisi_01_protein_novae.py
========================
Protein (Novae) Modality için LISI Hesaplama
----------------------------------------------
Strateji:
  ...
  - Embedding: global_9M_embeddings.npy içinde sütunlar 30:94 → Protein (Novae, 64 boyut)
...
N_CLISI = 150_000   # cLISI örneklem büyüklüğü
N_ILISI = 200_000   # iLISI örneklem büyüklüğü
N_NEIGHBORS = 90
RANDOM_STATE = 42

print("=" * 60)
print("  PROTEIN (NOVAE) LISI - STRATIFIED SAMPLING")
print("=" * 60)
...
# ─────────────────────────────────────────────
# cLISI filtresi: once isin() ile GT satirlari bul (9.4M uzerinde hizli hash)
# sonra sadece o ~1M satira .map() uygula
# ─────────────────────────────────────────────
gt_set = set(gt_dict.keys())  # O(1) lookup icin set
mask_gt = meta_full['uid'].isin(gt_set)  # bool mask, hizli
...
del gt_set, mask_gt  # artik gerekmiyor
gc.collect()
...
def stratified_sample_with_min(df, group_col, n_total, min_per_group=50,
                                random_state=42):
    """
    group_col'a gore orantili stratified orneklem yapar.
...
def stratified_sample_proportional(df, group_col, n_total, random_state=42):
    ...
    Her grubun agirliği toplam veriyle orantili kalir.
    """
    groups = df.groupby(group_col)
    samples = []
    for name, grp in groups:
...
# ─────────────────────────────────────────────
# 5. EMBEDDİNG SÜTUNLARI: PROTEIN = 30:94
# ─────────────────────────────────────────────
print("\n[5/6] Protein embedding'leri RAM'e aliniyor...")

# meta_clisi['index'] orijinal meta_full satir numarasını içeriyor
clisi_idx = meta_clisi['index'].to_numpy()
ilisi_idx = meta_ilisi.index.to_numpy()

prot_clisi = np.array(embeddings_full[clisi_idx, 30:])
prot_ilisi = np.array(embeddings_full[ilisi_idx, 30:])
...
t0 = time.time()
nn_ilisi = get_nn_results(prot_ilisi, N_NEIGHBORS)
...
prot_ilisi_mean = float(np.mean(ilisi_scores))
print(f"  iLISI tamamlandi: {prot_ilisi_mean:.4f}  ({time.time()-t0:.1f} sn)")
del prot_ilisi, nn_ilisi; gc.collect()

# --- cLISI (cell type purity) ---
print("  cLISI (cell type purity) hesaplaniyor...")
t0 = time.time()
nn_clisi = get_nn_results(prot_clisi, N_NEIGHBORS)
...
prot_clisi_mean = float(np.mean(clisi_scores))
print(f"  cLISI tamamlandi: {prot_clisi_mean:.4f}  ({time.time()-t0:.1f} sn)")
del prot_clisi, nn_clisi; gc.collect()

# ─────────────────────────────────────────────
# SONUÇLAR
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
print(f"  PROTEIN (NOVAE)  |  iLISI: {prot_ilisi_mean:.4f}  |  cLISI: {prot_clisi_mean:.4f}")
print("=" * 60)

md_path = OUT_DIR / "LISI_Protein_Novae.md"
with open(md_path, "w", encoding="utf-8") as f:
    f.write("# Protein (Novae) LISI Sonuclari\n\n")
    ...
    f.write("|--------|-------|-------|\n")
    f.write(f"| iLISI (Batch Mixing) | **{prot_ilisi_mean:.4f}** | Yuksek = iyi karisim |\n")
    f.write(f"| cLISI (Cell Type Purity) | **{prot_clisi_mean:.4f}** | Dusuk = iyi saflik |\n")
