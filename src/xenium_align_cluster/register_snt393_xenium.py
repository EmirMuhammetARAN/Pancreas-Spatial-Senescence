"""Register the 37-year SNT393-top PhenoCycler section to Xenium.
...
def estimate_affine(pc_crop: np.ndarray, xenium_dapi: np.ndarray) -> tuple[np.ndarray, str, dict[str, float]]:
    ...
    if best is None:
        raise RuntimeError("No robust DAPI registration was found. Inspect the selected SNT393 crop.")
    return best
...
def write_qc_images(output_dir: Path, xenium: np.ndarray, pc_crop: np.ndarray, pc_to_xenium_level: np.ndarray) -> None:
    ...
    cv2.imwrite(str(output_dir / "phenocycler_top_dapi.png"), percentile_uint8(pc_crop))
